...
{
    /* add your code here */
	ListNode *temp = *ptrHead;	//ptrHead = ll->head
	ListNode *prev = NULL;
	...
	while(temp->next !=NULL)
	{
		...
		{
			prev = temp;		//이전 노드를 현재 노드로 설정
			max = temp->next;	//max는 현재 노드의 다음 노드로 업데이트
		}
		temp = temp->next;	
	}
	if(max != *ptrHead)		//만약에 max의 위치가 head가 아니라면
	{
		prev->next = max->next;	//이전 노드의 next를 max 노드의 next로 설정
		max->next = *ptrHead;	//ptrHead를 max의 다음 노드로 업데이트
		*ptrHead = max;			//max를 ptrHead로 설정
	}
	return 0;
}
