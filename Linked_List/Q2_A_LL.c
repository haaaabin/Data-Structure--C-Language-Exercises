...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
	...
	{
		ll2->head = L2->next;	//ll2의 헤드를 L2의 다음 노드로 업데이트
        L2->next = L1->next;	//L2의 다음 노드를 L1의 다음 노드로 설정
        L1->next = L2;			//L1의 다음 노드를 L2로 설정

        L1 = L1->next->next;	//L1의 노드를 L1의 다음 다음 노드로 설정
        L2 = ll2->head;			//L2의 노드를 ll2의 헤드로 설정
