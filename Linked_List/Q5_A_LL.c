...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	...
	while(temp2 != NULL)			//리스트의 끝에 도달할 때까지
	{
		temp2 = temp2->next;		//temp2 한 칸 앞으로 이동
		if(temp2 !=	NULL)			//이동한 temp2가 null이 아니라면
		{
			temp1 = temp1->next;	//temp1는 한 노드 더 이동
			temp2 = temp2->next;	//temp2는 한 노드 더 이동(총 2칸 이동)
									// -> temp2가 리스트 끝에 도달하면 temp1은 중간에 위치
		}
	}
	resultFrontList->head = ll->head;
	resultFrontList->size = ll->size/2;	
	resultBackList->head = temp1->next;	//temp1 리스트의 중간에 위치하기 때문에 next를 head로
	resultBackList->size = ll->size - resultFrontList->size;

	temp1->next =NULL;	//temp1의 다음 노드를 NULL -> 두 리스트 완전히 분리
